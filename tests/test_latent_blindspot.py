...
def test_the_published_median_is_the_median_on_an_even_sample():
    """The guard on the bug this file's numbers were published with.

    `sorted(v)[len(v) // 2]` is the *upper* median on an even sample, and the unanimity
    summary is even by construction -- five slice draws times two slip rates. It published
    0.15 for `margin` and `posterior` where both are 0.125, always in the rule's favour.

    Written against a known-even sample with a known answer rather than against the
    artifact, because an artifact regenerated by the same wrong function agrees with itself.
    """
    from measure_latent_blindspot import _median

    assert _median([0.05, 0.05, 0.1, 0.1, 0.1, 0.15, 0.15, 0.15, 0.15, 0.25]) == 0.125
    assert _median([0.0, 1.0]) == 0.5
    assert _median([3.0, 1.0, 2.0]) == 2.0, "the odd case must not have moved"


def test_the_unanimity_summary_is_an_even_sample_and_still_reads_true():
    """The bug above was reachable only because this sample is even, so pin that too.

    If the grid ever stops being draws x slip rates, the even case may stop being exercised
    and the guard above would keep passing while nothing published depended on it.
    """
    payload = artifact("latent_blindspot.json")
    draws = {rule: stats["draws"] for rule, stats in payload["unanimity_precision"].items()}
    assert draws, "no unanimity summary to check"
    for rule, n in draws.items():
        assert n % 2 == 0, f"{rule} is no longer an even sample; the median guard is idle"
    for rule, stats in payload["unanimity_precision"].items():
        if stats["median"] is None:
            continue
        assert stats["min"] <= stats["median"] <= stats["max"], rule


def test_the_separability_verdict_rests_only_on_cells_that_could_fail():
    """A row where both constructions build the same fleet cannot falsify anything.

    At `n_blind == 0` there is no blind analyst, so `latent_blind_fleet` and `blind_fleet`
    return identical tuples and the row reads as agreement whatever the statistic does.
    Counting it as evidence inflates the claim's support by a cell it could never lose.
    """
    payload = artifact("latent_blindspot.json")
    spread = payload["dispersion_spread"]
    assert any(not row["informative"] for row in spread), (
        "the unblinded reference row is gone; either the grid changed or it stopped "
        "being published, and the verdict's denominator no longer means what it says"
    )
    testable = [row for row in spread if row["informative"]]
    assert payload["dispersion_cells_that_could_falsify"] == len(testable)
    assert len(testable) < len(spread), "every row is informative; the marker is doing nothing"
    assert all(row["n_blind"] > 0 for row in testable)
    assert all(row["channel_inside_latent_spread"] for row in testable)
...
def test_a_size_the_corpus_cannot_host_is_skipped_and_said_so(small, monkeypatch, caplog):
    """An unhostable size must leave a trace, rather than a row that is merely absent.

    The sweep used to learn this by catching the refusal from every draw, which emptied the
    row and dropped it: the artifact then published a sweep with no mention of the size it
    skipped, and a shorter table was the only evidence. Asked up front against the eligible
    pool instead, so the skip is a decision with a record rather than an absence.
    """
    import measure_latent_blindspot as m

    tasks, proposals, truth = small
    monkeypatch.setattr(m, "SLICE_SIZES", (10, 10_000))
    with caplog.at_level("WARNING"):
        rows = m.slice_sweep(tasks, proposals, truth, fleet=5, seed=7, null_draws=1)
    assert {r.size for r in rows} == {10}, "the hostable size did not survive the skip"
    assert any("size_unhostable" in r.message for r in caplog.records), (
        "the skipped size left no record; an absent row is indistinguishable from a row "
...
def test_a_refusal_the_corpus_owns_is_not_confused_with_one_a_draw_owns():
    ...
    Both read `"carriage gap" not in str(refusal)` and so classified the balance rule's
    exhaustion -- whose message mentions no carriage gap -- as structural. One retried it
    correctly anyway; the other re-raised and would have aborted a sweep over a refusal
    another seed can fix.
    """
    corpus = ChannelUnusableError("this corpus has 20", structural=True)
    draw = ChannelUnusableError("no balanced slice of 20 in 50 draws", structural=False)
    assert corpus.structural is True
    assert draw.structural is False
    assert ChannelUnusableError("unspecified").structural is True, (
        "the default must be the conservative one: an unclassified refusal should stop a "
        "caller rather than be retried into `attempts` identical failures"
    )
