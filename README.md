...
Writes three files: `corpus.jsonl`, the `manifest.json` that certifies it, and a
`croissant.json` describing it. The command refuses to write anything when the
corpus fails its own gate, because a citable artifact its own gate rejected is
worse than no artifact.

Croissant is the MLCommons metadata format, and as of the 2026 NeurIPS
Evaluations and Datasets track a dataset submission has to carry one with
Responsible AI metadata inside it. That record is generated from the manifest
rather than maintained by hand: a separate metadata file is a second source of
truth, and it drifts on the first change.

A generated corpus stresses the format in one specific way. Croissant assumes a
`contentUrl` pointing at a file that already exists, and a Pharos corpus does not
exist until someone runs the generator. So the record identifies the artifact by
what reproduces it, which is generator version, commit, seed, and config, and
carries the `sha256` of one instantiation. That is what turns "rerun this and you
get the same bytes" from a claim into a check.

The record is validated by the official `mlcroissant` library in the test suite,
not by reading it. The first version here was written straight from the
specification and looked right: every property present, every shape correct. It
was invalid anyway, because an abridged `@context` omitted `column`, so every
field's `extract` resolved to nothing. Reading cannot catch that and one call to
the validator did.
...
Every result is regenerated by one target, into `results/`, tracked in git:

```bash
make results     # needs Ollama serving the model named in each script
```

Each artifact carries a provenance block naming the version, commit, whether the
